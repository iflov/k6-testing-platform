...
function trackDegradation(responseTime) {
  ...
  if (baselineMetrics.responseTime && requestCount > 500) {
    ...
    const degradationPercentage =
      (degradation / baselineMetrics.responseTime) * 100;
...
function textSummary(data, options) {
  ...
  if (metrics) {
    const testDurationHours =
      ((data.state && data.state.testRunDurationMs) || 0) / 1000 / 60 / 60;
    ...
    summary += `  • Initial Response Time: ${
      baselineMetrics.responseTime
        ? baselineMetrics.responseTime.toFixed(2)
        : "N/A"
    }ms\n`;
    ...
    const avgDegradation =
      (metrics.response_time_degradation &&
        metrics.response_time_degradation.avg) ||
      0;
    const maxDegradation =
      (metrics.response_time_degradation &&
        metrics.response_time_degradation.max) ||
      0;
    const degradationPercentage =
      (metrics.performance_degradation_percentage &&
        metrics.performance_degradation_percentage.value) ||
      0;

    summary += `  • Average Degradation: ${avgDegradation.toFixed(2)}ms\n`;
    summary += `  • Max Degradation: ${maxDegradation.toFixed(2)}ms\n`;
    summary += `  • Degradation Percentage: ${degradationPercentage.toFixed(
      2
    )}%\n`;

    summary += "\n💾 Resource Usage:\n";
    summary += `  • Average Memory: ${
      metrics.memory_usage_mb && metrics.memory_usage_mb.avg
        ? metrics.memory_usage_mb.avg.toFixed(2)
        : "N/A"
    } MB\n`;
    summary += `  • Peak Memory: ${
      metrics.memory_usage_mb && metrics.memory_usage_mb.max
        ? metrics.memory_usage_mb.max.toFixed(2)
        : "N/A"
    } MB\n`;
    summary += `  • Average CPU: ${
      metrics.cpu_usage_percentage && metrics.cpu_usage_percentage.avg
        ? metrics.cpu_usage_percentage.avg.toFixed(2)
        : "N/A"
    }%\n`;
    summary += `  • Peak CPU: ${
      metrics.cpu_usage_percentage && metrics.cpu_usage_percentage.max
        ? metrics.cpu_usage_percentage.max.toFixed(2)
        : "N/A"
    }%\n`;
    ...
    summary += `  • Connection Errors: ${
      (metrics.connection_errors && metrics.connection_errors.count) || 0
    }\n`;
    summary += `  • Resource Exhaustion Events: ${
      (metrics.resource_exhaustion && metrics.resource_exhaustion.count) || 0
    }\n`;
    ...
    if (avgDegradation < 50 && errorRate < 0.01 && exhaustionCount === 0) {
      summary +=
        "  ✅ EXCELLENT: System is highly stable for long-term operation\n";
    } else if (
      avgDegradation < 100 &&
      errorRate < 0.02 &&
      exhaustionCount < 10
    ...
    if (avgDegradation > 100) {
      summary +=
        "  ⚠️ Possible memory leak detected (response time degradation)\n";
    }
    if ((metrics.connection_errors && metrics.connection_errors.count) > 50) {
      summary += "  ⚠️ Connection pooling issues detected\n";
    }
    if (
      (metrics.resource_exhaustion && metrics.resource_exhaustion.count) > 20
    ) {
      summary += "  ⚠️ Resource management issues detected\n";
    }
    if (degradationPercentage > 30) {
      summary += `  ⚠️ Performance degraded by ${degradationPercentage.toFixed(
        1
      )}% over time\n`;
    }
  }
