...
export default function () {
  ...
  if (scenario < 0.30) {
    ...
    group('Heavy Read Operations', () => {
      ...
      if (currentSuccessRate < 0.5 && lastSuccessRate >= 0.5 && !breakingPointDetected) {
    ...
    group('Heavy Write Operations', () => {
      ...
      for (let i = 0; i < 3; i++) {
        orders.push({
          ...
          productIds: Array.from({length: 5}, () => Math.floor(Math.random() * 100) + 1),
          quantities: Array.from({length: 5}, () => Math.floor(Math.random() * 3) + 1),
      ...
      orders.forEach((order) => {
        ...
        const success = check(res, {
          ...
          'heavy write response time < 5000ms': (r) => r.timings.duration < 5000,
        });
    ...
    group('Complex Queries', () => {
      ...
      complexQueries.forEach((query) => {
        ...
        const success = check(res, {
          ...
          'complex query response time < 4000ms': (r) => r.timings.duration < 4000,
        });
    ...
    group('Database Intensive', () => {
      ...
      const res = http.get(`${BASE_URL}/api/reports/generate?type=sales&format=json`, {
        timeout: '10s',
      });
      
      const success = check(res, {
        'report generation status is 200': (r) => r.status === 200,
        'report generation response time < 8000ms': (r) => r.timings.duration < 8000,
      });
    ...
    group('Health Check', () => {
      ...
      if (!success && currentVUs > 100) {
        console.log(`Health check failed at ${currentVUs} VUs`);
      }
    });
  }
