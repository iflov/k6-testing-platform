  ...
  if (loading) {
    return (
      <div className="bg-white rounded-lg shadow-md p-6">
        <h2 className="text-2xl font-semibold text-gray-800 mb-4">Loading Test History...</h2>
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="text-2xl font-bold text-gray-900">{testRuns.length}</div>
          </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">VUs</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredAndSortedTests.map((test) => (
                  <tr key={test.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900">{test.scenario}</td>
                    <td className="px-4 py-3 text-sm">
                      <span className={`px-2 py-1 text-xs font-medium rounded ${
                        test.httpMethod === 'GET' ? 'bg-blue-100 text-blue-800' :
                        test.httpMethod === 'POST' ? 'bg-green-100 text-green-800' :
                        test.httpMethod === 'PUT' ? 'bg-yellow-100 text-yellow-800' :
                        test.httpMethod === 'DELETE' ? 'bg-red-100 text-red-800' :
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900">
                      <div className="truncate max-w-xs" title={test.targetUrl + test.urlPath}>
                        {test.urlPath || '/'}
                      </div>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-900">{test.vus}</td>
                    <td className="px-4 py-3 text-sm text-gray-900">
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-2xl font-semibold text-gray-900">Test Details</h3>
                <button
                ...
                <div>
                  ...
                  <dl className="space-y-2">
                    <div>
                      ...
                      <dd className="text-sm font-mono text-gray-900">{selectedTest.testId}</dd>
                    </div>
                    <div>
                      <dt className="text-sm text-gray-500">Scenario</dt>
                      <dd className="text-sm text-gray-900">{selectedTest.scenario}</dd>
                    </div>
                    <div>
                      <dt className="text-sm text-gray-500">Execution Mode</dt>
                      <dd className="text-sm text-gray-900">{selectedTest.executionMode}</dd>
                    </div>
                    <div>
                      <dt className="text-sm text-gray-500">Virtual Users</dt>
                      <dd className="text-sm text-gray-900">{selectedTest.vus}</dd>
                    </div>
                    <div>
                      <dt className="text-sm text-gray-500">Duration</dt>
                      <dd className="text-sm text-gray-900">{selectedTest.duration || 'N/A'}</dd>
                    </div>
                    <div>
                      <dt className="text-sm text-gray-500">Iterations</dt>
                      <dd className="text-sm text-gray-900">{selectedTest.iterations || 'N/A'}</dd>
                    </div>
                  </dl>
                </div>
                ...
                <div>
                  ...
                  <dl className="space-y-2">
                    <div>
                      ...
                      <dd className="text-sm text-gray-900">{selectedTest.httpMethod}</dd>
                    </div>
                    <div>
                      <dt className="text-sm text-gray-500">Target URL</dt>
                      <dd className="text-sm text-gray-900 break-all">{selectedTest.targetUrl}</dd>
                    </div>
                    <div>
                      <dt className="text-sm text-gray-500">URL Path</dt>
                      <dd className="text-sm text-gray-900">{selectedTest.urlPath || '/'}</dd>
                    </div>
                    <div>
                      <dt className="text-sm text-gray-500">Status</dt>
                      <dd>
                        <span className={`px-2 py-1 text-xs font-medium rounded ${getStatusColor(selectedTest.status)}`}>
                          {selectedTest.status}
                        </span>
                      </dd>
                    </div>
                    <div>
                      ...
                      <dd className="text-sm text-gray-900">{formatDate(selectedTest.startedAt)}</dd>
                    </div>
                    <div>
                      <dt className="text-sm text-gray-500">Completed At</dt>
                      <dd className="text-sm text-gray-900">
                        {selectedTest.completedAt ? formatDate(selectedTest.completedAt) : 'N/A'}
                      </dd>
                    </div>
                  </dl>
                </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="bg-gray-50 rounded p-3">
                      ...
                      <div className={`text-xl font-semibold ${
                        selectedTest.testResult.errorRate > 5 ? 'text-red-600' : 'text-green-600'
                      }`}>
