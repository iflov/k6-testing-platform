...
export default function () {
  ...
  if (consecutiveFailures >= 10 && !breakpointFound) {
    ...
    console.log(`BREAKPOINT DETECTED at ${currentVUs} VUs`);
    console.log(`System started failing consistently at this load level`);
  }
...
function textSummary(data, options) {
  ...
  if (metrics) {
    ...
    if (breakpointVUs > 0) {
      ...
      summary += `  • Recommended max load: ${Math.floor(breakpointVUs * 0.8)} VUs (80% of breakpoint)\n`;
