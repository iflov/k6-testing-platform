...
export default function () {
  ...
  if (currentVUs > 50 && !inSpike) {
    ...
    console.log(`Spike detected: ${currentVUs} VUs at ${new Date().toISOString()}`);
  } else if (currentVUs <= 20 && inSpike) {
    recoveryStartTime = currentTime;
    inSpike = false;
    recovering = true;
    const spikeDuration = currentTime - spikeStartTime;
    console.log(`Spike ended after ${spikeDuration}ms`);
  ...
  if (scenario < 0.40) {
    ...
    group('Order Processing', () => {
      ...
      const success = check(res, {
        'order processed': (r) => r.status === 201 || (inSpike && r.status === 503),
        'order response time': (r) => r.timings.duration < (inSpike ? 5000 : 2000),
      });
      
      if (!success && inSpike) {
        spikeErrors.add(1);
        console.log(`Order failed during spike: ${res.status}`);
      }
    ...
    group('Search Operations', () => {
      ...
      const success = check(res, {
        'search works': (r) => r.status === 200 || (inSpike && r.status === 503),
    ...
    group('Health Monitoring', () => {
      ...
      if (!success && inSpike) {
        console.log(`Health check degraded during spike: ${res.timings.duration}ms`);
      }
      ...
      if (recovering && baselinePerformance && res.timings.duration < baselinePerformance * 1.2) {
        systemRecovered.add(1);
      }
...
function textSummary(data, options) {
  ...
  if (metrics) {
    ...
    if (spikeErrorRate < 0.05 && recoveryRate > 0.8) {
      ...
      summary += '  ⚠️ Good: System handled spikes with acceptable degradation\n';
    } else if (spikeErrorRate < 0.30 && recoveryRate > 0.4) {
    ...
    summary += `  • Recovery Success Rate: ${(recoveryRate * 100).toFixed(2)}%\n`;
  }
